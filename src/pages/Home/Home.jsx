...
const Home = () => {
  ...
  const TokensData = [
    { logo: PoxLogo, label: "POX", acounts:23454523564, transactions:734756734 },
    { logo: UsdxLogo, label: "USDX", acounts:23454523564, transactions:734756734 },
    { logo: UviLogo, label: "UVI", acounts:23454523564, transactions:734756734 },
    { logo: SulLogo, label: "SUL", acounts:23454523564, transactions:734756734 },
  ];

  return (
    <div className="min-h-screen text-white mt-10">
      <div className="flex flex-col items-center">
        <p className="text-white font-bold text-5xl text-center pb-4">
        ...
        <div className="flex justify-center items-center space-x-6 my-6 text-lg">
          <p>Trending Search:</p>
          {trendingData.map((item, index) => (
            <div key={index} className="flex items-center space-x-2">
              <img src={item.logo} alt={`${item.label}-logo`} className="w-8 h-8" />
              <p>{item.label}</p>
            </div>
          ))}
        </div>
        ...
        {/* Blocks Section */}
        <div className="w-full grid grid-cols-2 lg:grid-cols-4 gap-6 my-6">
          {blockData.map((block, index) => (
            <div key={index} className="reusable-box px-6 py-4 flex justify-between items-center space-x-6 text-sm">
              <div className="flex items-center space-x-6">
                <img src={block.image} alt={block.title} className="w-8 h-8" />
                <div>
                  <p className="pb-1 text-white text-opacity-50">{block.title}</p>
                  <p className="font-medium">{formatNumberWithCommas(block.value)}</p>
                </div>
              </div>
              <div>
                <p className="pb-1 text-white text-opacity-50">24 hrs</p>
        ...
        <div className="w-full flex items-center justify-around rounded-2xl my-2 px-6 py-4" 
     style={{ background: 'linear-gradient(90deg, #080808 -5.16%, #1A3013 10.61%, #165401 47.09%, #0E2A05 89.55%, #080808 108.19%)' }}>
  <p className="text-opacity-50 text-white">
    Current / MaxTPS: <span className="text-opacity-100 text-white">0/1000</span>
  </p>
  <p className="text-opacity-50 text-white">
  ...
  <p className="text-opacity-50 text-white">
    Total Contracts: <span className="text-opacity-100 text-white">85</span>
  </p>
</div>
      </div>
      <div className="grid grid-cols-[70%_28%] gap-6 my-4">
      <div className="grid grid-cols-[78%_20%] gap-6 bg-[#151515] border-[1px] border-[#434343] rounded-3xl px-6 py-5 shadow-lg">
        <div>
        ...
        <div>
          ...
          <div className="flex flex-row items-center justify-between mb-4">
            <div>
              <p className="text-white text-opacity-50 text-sm font-medium pb-1">Market Cap</p>
              <p>$ 4.75M</p>
            </div>
            <div>
              <p className="text-white text-opacity-50 text-sm font-medium pb-1">Volume (24hr)</p>
              <p>164.98%</p>
            </div>
          </div>
          {/* Supply / Staked / BandWidth */}
          <div className="bg-gradient-to-r from-[#124202] via-[#124202] to-[#151515] px-4 py-2 w-52 rounded-l-xl mb-2">
            <p className="text-white text-opacity-50 text-sm font-medium pb-2">Supply</p>
            <p>{formatNumberWithCommas(4634673)} POX</p>
          </div>
          <div className="bg-gradient-to-r from-[#124202] via-[#124202] to-[#151515] px-4 py-2 w-52 rounded-l-xl mb-2">
            <p className="text-white text-opacity-50 text-sm font-medium pb-2">Staked</p>
            <p>{formatNumberWithCommas(4634673)}</p>
          </div>
          <div className="bg-gradient-to-r from-[#124202] via-[#124202] to-[#151515] px-4 py-2 w-52 rounded-l-xl mb-2">
            <p className="text-white text-opacity-50 text-sm font-medium pb-2">Bandwidth: <span className="text-white ">{formatNumberWithCommas(4634673)}</span></p>
            <p className="text-white text-opacity-50 text-sm font-medium pb-2">Energy: <span className="text-white ">{formatNumberWithCommas(4634673)}</span></p>
          </div>
        </div>
      </div>

  <div className="bg-[#151515] border-[1px] border-[#434343] rounded-3xl px-6 py-5 shadow-lg">
  {/* Header Section */}
  <div className="flex flex-row items-center justify-between mb-4">
